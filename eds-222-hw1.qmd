...
I used the "here" package to help create a file path from my home directory to my data folder, "HW1." I successfully read the data by starting at the "HW1" folder and calling the file name I want to assign to each variable.

```{r}
# Read in data
crowdsourced <- readRDS(here::here("HW1", "airpol-PK-crowdsourced.RDS"))
govt <- readRDS(here::here("HW1", "airpol-PK-govt.RDS"))
```
...
The population in this study is Lahore, Pakistan. This city has approximately 11.1 million people.

2.  What are the *samples* in this setting? Please be precise.

The samples in this study consist of air quality monitors that were crowdsourced from volunteers' homes, as well as monitors installed by government officials at select locations across Lahore that are deemed "cleaner."
...
# Collect a random sample of 1000 observations
samp_1000 <- combined %>%
  sample_n(1000)
...
# Collect a random sample of 200 observations
samp_200 <- combined %>%
  group_by(strat_2) %>%
  sample_n(200) %>%
  ungroup()
...
The stratified random sample seems to best match the satellite data because it does not rely solely on government monitors. The purpose of the strata is to reduce the influence of government monitors on assessing air quality for the entire city, as they only cover a small localized portion. This approach provides the opportunity to sample and study other areas of Lahore. Overall, I recommend that the Prime Minister advocates for the people by expanding government monitors and endorsing the use of the stratified dataset for reporting.
